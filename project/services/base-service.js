  ...
  async findAll() {
    return new Promise((resolve, reject) => {
      fs.readFile(this.dbPath, 'utf8', async (err, file) => {
        ...
        const parsed = Flatted.parse(file);
        const items = parsed.map(this.model.create)
  ...
  async saveAll(items) {
    return new Promise((resolve, reject) => {
      fs.writeFile(this.dbPath, Flatted.stringify(items), (err, file) => {
        ...
        if (err) {
          console.log("Failed file write in SaveAll");
          return reject(err)
        }
        console.log("Successful file write in SaveAll");
        resolve()
      })
    })
  }
